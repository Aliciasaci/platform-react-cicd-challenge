...
import { getEtablissements, returnNull } from "../services/prestataires.service";
        ...
        if (etablissements) {
            setLocations(
                etablissements.map((etablissement) => ({
                    ...
                    images: etablissement.imageEtablissements,
                }))
            );
        }
    }, [etablissements]);
    ...
    return (
        <div className='w-full grid' style={{ gridTemplateColumns: "1fr 1.5fr" }}>
            <div className='h-[93vh] overflow-y-auto'>
                {etablissements?.map((etablissement) => (
                    <PublicCard key={etablissement.id} etablissement={etablissement} isLoading={isLoading} />
                ))}
            </div>
