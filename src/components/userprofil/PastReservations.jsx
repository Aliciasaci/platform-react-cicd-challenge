    ...
    const fetchReservationsInfos = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const mappedReservations = response.data.reservationsClient.map(
            (reservation) => ({
              ...reservation,
              jour_2: new Date(
                reservation.jour.split("/").reverse().join("/")
              ).setHours(
          ...
          const filteredReservations = mappedReservations.filter(
            (reservation) => reservation.jour_2 < today
          );
  ...
  return (
    <div className="h-full w-screen bg-gray-100">
      <div className="mt-10 ml-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <a href="#">
          <h5 className="text-xl text-gray-800 mb-4">
            {t("Reservation_Past")}
          </h5>
        </a>
        {pret ? (
