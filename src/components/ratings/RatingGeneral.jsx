...
export default function RatingGeneral({ notes }) {
    const calculateAverages = ({ notes }) => {
        ...
        let moyenneGenerale = totalNotes / totalCritere;
        if (isNaN(moyenneGenerale)) moyenneGenerale = 0;
    ...
    // const setMoyenneGenerale = async (e) => {
    //  try {
    //   const response = await axios.patch(`https://127.0.0.1:8000/api/prestation/`, {
                                    ...
                                    <dd className='flex items-center mb-3'>
                                        <div className='w-full bg-gray-200 rounded h-2.5 dark:bg-gray-700 me-2'>
                                            <div
                                                ...
                                                style={{ width: `${moyennesParCritere[critere] * 20}%` }}
                                            ></div>
                                        </div>
                                        <span className='text-sm font-medium text-gray-500 dark:text-gray-400'>
                                            {moyennesParCritere[critere].toFixed(1)}
                                        </span>
                                    </dd>
                                </dl>
                            ))}
