            ...
            if (userNotes.length > 0) {
                userNotes.forEach((note) => {
                    setRatings((prevRatings) => ({
                        ...prevRatings,
                        [note.critere.titre]: { id: note.id, note: note.note },
                    }));
                });
            }
        };
            ...
            <Card className='w-full'>
                ...
                {criteres.map((critere, index) => (
                    <div key={index} className='flex' style={{ display: "flex", flexWrap: "wrap" }}>
                        <p className='text-light text-black mb-1' style={{ flexBasis: "100%" }}>
                            {critere.titre}
                        </p>
                                ...
                                <label key={index}>
                                    ...
                                    <FaStar
                                        ...
                                        color={
                                            currentRating <=
                                            (hover[critere.titre] ||
                                                (ratings[critere.titre] && ratings[critere.titre]["note"]))
                                                ? "#ffc107"
                                                : "#e4e5e9"
                                        }
                        ...
                        {ratings[critere.titre] && (
                            <p className='text-gray-500 ml-2 font-semibold'> {ratings[critere.titre]["note"]}</p>
                        )}
                        <br />
                    </div>
                ))}
